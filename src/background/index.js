...
function broadcastProgress(progress) {
  chrome.runtime
    .sendMessage({
      type: MESSAGE_TYPES.SCAN_PROGRESS_EVENT,
      progress,
    })
    .catch(() => {
...
async function handleMessage(message) {
  switch (message.type) {
    ...
    case MESSAGE_TYPES.SCAN_START: {
      scanService.start(broadcastProgress);
    ...
    case MESSAGE_TYPES.TREE_CHILDREN_GET: {
      const children = await driveRepository.getChildren(
        message.parentId ?? DRIVE_ROOT_ID,
      );
    ...
    case MESSAGE_TYPES.SUBTREE_TEXT_GET: {
      const descendants = await driveRepository.getDescendants(
        message.folderId,
      );

      const paths = descendants
        .map((n) => n.path.split(" | ")[0]) // Extract only the path, before " | "
        .filter(Boolean)
...
function applyFilters(nodes, filters) {
  return nodes.filter((node) => {
    ...
    if (filters.mimeType && node.mimeType !== filters.mimeType) {
      return false;
    }

    if (filters.parentPath && !node.path.startsWith(filters.parentPath)) {
      return false;
    }
